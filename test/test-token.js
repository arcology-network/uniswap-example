...
async function main() {
  ...
  for(i=0;i<tokens.length;i=i+2) {
    ...
    balance = await tokenInsArray[i+1].balanceOf(accounts[i+1].address);
    formattedBalance = ethers.utils.formatUnits(balance, decimals);
    console.log(`Balance of account ${accounts[i+1].address}: ${formattedBalance} ${tokens[i].name}`);
  }
  ...
  for (i=0;i<tokens.length;i=i+2) {
    await addLiquidity(nonfungiblePositionManager,tokenInsArray[i],tokenInsArray[i+1],fee,"1","1",accounts[i]);
  }
